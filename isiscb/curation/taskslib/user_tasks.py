...
@shared_task
def reindex_users(user_id, filter_params_raw, task_id=None, object_type='USER'):
    ...
    try:    # Report all exceptions as a task failure.
        for i, obj in enumerate(queryset):
            ...
            haystack.connections[settings.HAYSTACK_DEFAULT_INDEX].get_unified_index().get_index(UserProfile).update_object(obj)

        task.state = 'SUCCESS'
        task.save()
    except Exception as E:
        print('bulk_update_users failed for %s' % filter_params_raw, end=' ')
        print(E)
        task.state = 'FAILURE'
        task.save()
