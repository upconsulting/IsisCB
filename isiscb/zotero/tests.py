...
#         citations = ingest.process(papers, instance)
#
...
class TestImportMethods(TestCase):
    def test_get_dtype(self):
        """
        :func:`zotero.ingest._get_dtype` is a private function that
    ...
    def test_get_pages_data(self):
        ...
        data = ingest._get_pages_data({'pages':[u'555']})
    ...
    def test_generate_generic_acrelationss(self):
        ...
        results = ingest.generate_generic_acrelations(
    ...
    def test_generate_reviewed_works_relations(self):
        ...
        try:
            r = ingest.generate_reviewed_works_relations({}, draftcitation)
        except:
        ...
        try:
            r = ingest.generate_reviewed_works_relations(data, draftcitation)
        except:
            self.fail("Should not choke on nonsensical data, but rather fail"
                      " quietly.")
        ...
        books = {'12345678': alt_draftcitation}
        r = ingest.generate_reviewed_works_relations(data, draftcitation, books)
    ...
    def test_generate_book_chapter_relations(self):
        ...
        result = ingest.generate_book_chapter_relations(data, draftcitation)
        ...
        books = {'9781874267621': alt_draftcitation}
        r = ingest.generate_book_chapter_relations(data, draftcitation, books)
        self.assertEqual(len(r), 1, "If a matching citation is found in this"
                                    " accession, it should be associated.")
        draftcitation.refresh_from_db()
    ...
    def test_generate_part_of_relations(self):
        ...
        result = ingest.generate_part_of_relations(data, draftcitation)
        self.assertEqual(len(result), 2, "Should yield two records")


    def test_generate_citation_linkeddata(self):
        """
        :func:`zotero.ingest.generate_citation_linkeddata` creates new
        ...
        linkeddata = ingest.generate_citation_linkeddata(data, draftcitation)
        ...
        try:
            ingest.generate_citation_linkeddata({}, draftcitation)
        except:
            self.fail("Should not choke when no linkeddata are present.")

    def test_generate_draftcitation(self):
        """
        :func:`zotero.ingest.generate_draftcitation` creates a new
        ...
        accession = ImportAccession.objects.create(name=u'test')
        draft_citation = ingest.generate_draftcitation(data, accession)
    ...
    def test_find_mapped_authority(self):
        """
        :func:`zotero.ingest._find_mapped_authority` attempts to find an
        ...
        candidate = ingest._find_mapped_authority('Whoops', '13')
        self.assertEqual(candidate, authority,
                         "Should return the corresponding authority.")

        self.assertEqual(ingest._find_mapped_authority('Whoops', '12'), None,
                         "If not found, should return None.")

    def test_find_encoded_authority(self):
        """
        :func:`zotero.ingest._find_encoded_authority` attempts to find an
        ...
        candidate = ingest._find_encoded_authority('=56-55=', None)
        self.assertEqual(candidate, authority,
                         "Should return the corresponding authority.")

        candidate = ingest._find_encoded_authority('=6=', None)
        self.assertEqual(candidate, authority2,
                         "Should return the corresponding authority.")

        self.assertEqual(ingest._find_encoded_authority('=55-56=', None), None,
                         "If not found, should return None.")
    def test_find_explicit_authority(self):
        """
        :func:`zotero.ingest._find_explicit_authority` attempts to find an
        ...
        candidate = ingest._find_explicit_authority('CBA000113709', None)
        self.assertEqual(candidate, authority,
                         "Should return the corresponding authority when"
                         " the identifier is passed as the label.")

        candidate = ingest._find_explicit_authority(None, 'CBA000113709')
        self.assertEqual(candidate, authority,
                         "Should return the corresponding authority when"
    ...
    def test_generate_subject_acrelations(self):
        ...
        results = ingest.generate_subject_acrelations(data, draftcitation)
    ...
    def tearDown(self):
        for model in [DraftCitation, DraftAuthority, DraftACRelation,
                      ...
                      DraftAuthorityLinkedData, Authority]:
