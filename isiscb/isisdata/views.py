...
def home(request, template='isisdata/home.html', tenant_id=None):
    ...
    if tenant:
        featured_citations = featured_citations.filter(subject_ids__in=current_featured_authority_ids, owning_tenant=tenant.pk)
    else:
        featured_citations = featured_citations.filter(subject_ids__in=current_featured_authority_ids)
    featured_citations = featured_citations.filter(type__in=['Book', 'Article']).filter(abstract = Raw("[* TO *]")).filter(title = Raw("[* TO *]")).query.get_results()
